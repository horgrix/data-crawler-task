"""
xd_game_steam_weekly_hot_list 表的数据操作模块。
...
class XdGameSteamWeeklyHotlistDao(XdGameSteamDao):
    """xd_game_steam_weekly_hot_list 表 DAO"""

    # ========================
    # 建表 SQL
    # ========================
    CREATE_TABLE_SQL = """
    CREATE TABLE IF NOT EXISTS xd_game_steam_weekly_hot_list (
    ...
    # ========================
    UPSERT_SQL = """
    INSERT INTO xd_game_steam_weekly_hot_list
    ...
    SELECT start_ts, end_ts, `rank`, steam_id, region
    ...
    SELECT start_ts, end_ts, `rank`, steam_id, region
    FROM xd_game_steam_weekly_hot_list
    WHERE steam_id = %s AND region = %s AND start_ts >= %s AND start_ts <= %s
    ORDER BY start_ts DESC
    """
